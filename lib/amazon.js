...
function guessSeries(title) {
  ...
  let m = t.match(/\(([^)]+?),?\s+Book\s+#?([\d.]+)(?:\s+of\s+\d+)?\)/i) // (Series[,] Book 2)
  if (m) return mk(m[1], m[2])
  m = t.match(/\(Book\s+([\d.]+)\s+of\s+(?:the\s+)?([^)]+)\)/i) // (Book 2 of Series)
  if (m) return mk(m[2], m[1])
  m = t.match(/\(([^)]+?)\s+#([\d.]+)\)/) // (Series #2)
  if (m) return mk(m[1], m[2])
  m = t.match(/^(.*?)[,:]?\s+Book\s+([\d.]+)\b/i) // Series[,:] Book 2 ...
  ...
  function mk(name, num) {
    ...
    if (!name || /^(the|a|an)$/i.test(name)) return null
    return { name, num: parseFloat(num) }
  }
}

const seriesKey = (name) =>
  name.toLowerCase().replace(/^(the|a|an)\s+/, '').replace(/[^a-z0-9]+/g, ' ').trim()
