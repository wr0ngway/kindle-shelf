...
function createRemoteServer({ port, staticDir, getToken, api, log = () => {} }) {
  ...
  const server = http.createServer(async (req, res) => {
    try {
      ...
      // Re-authorize a device that lost its cookie (e.g. an installed PWA,
      // which keeps a separate cookie jar from the browser). The desktop
      // panel shows the token; pasting it here sets the same long-lived
      // cookie the QR flow would. Must run before the authed /api/ gate.
      if (p === '/api/authorize' && req.method === 'POST') {
        const chunks = []
        for await (const c of req) chunks.push(c)
        let token = null
        try { token = JSON.parse(Buffer.concat(chunks).toString()).token } catch {}
        if (!tokenEqual(token, getToken())) {
